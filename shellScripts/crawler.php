  ...
  if(empty($response['error']) OR $response['error'] === NULL) {
    foreach($response['items'] AS $item) {
      ...
      if(mysqli_num_rows($result) === 0) {
         ...
         */
        $stats['new']++;
        mysqli_query($dbl, "INSERT INTO `items` (`itemId`, `promoted`, `up`, `down`, `benis`, `created`, `image`, `thumb`, `fullsize`, `width`, `height`, `audio`, `extension`, `flags`, `username`, `mark`) VALUES ('".defuse($item['id'])."', '".($item['promoted'] ? 1 : 0)."', '".defuse($item['up'])."', '".defuse($item['down'])."', '".(defuse($item['up'])-defuse($item['down']))."', '".date('Y-m-d H:i:s', defuse($item['created']))."', '".defuse($item['image'])."', '".defuse($item['thumb'])."', '".defuse($item['fullsize'])."', '".defuse($item['width'])."', '".defuse($item['height'])."', '".($item['audio'] === TRUE ? 1 : 0)."', '".defuse(pathinfo($item['image'])['extension'])."', '".defuse($item['flags'])."', '".defuse($item['user'])."', '".defuse($item['mark'])."')") OR DIE(MYSQLI_ERROR($dbl));
