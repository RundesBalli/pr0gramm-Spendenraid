...
"<div class='col-x-4 col-s-4 col-m-1 col-l-1 col-xl-1' style='border-left: 6px solid #888888;'>ID</div>".PHP_EOL.
"<div class='col-x-8 col-s-8 col-m-2 col-l-2 col-xl-2'>Username</div>".PHP_EOL.
"<div class='col-x-8 col-s-8 col-m-3 col-l-3 col-xl-3'>Zeitpunkt</div>".PHP_EOL.
"<div class='col-x-4 col-s-4 col-m-2 col-l-2 col-xl-2'>PostID</div>".PHP_EOL.
"<div class='col-x-12 col-s-12 col-m-4 col-l-4 col-xl-4'>Text</div>".PHP_EOL.
"</div>".PHP_EOL;
 ...
 * Loganzeige
 */
$result = mysqli_query($dbl, "SELECT `log`.`id`, `users`.`username`, `log`.`timestamp`, `loglevel`.`color`, `log`.`postId`, `log`.`text` FROM `log` LEFT OUTER JOIN `users` ON `users`.`id`=`log`.`userId` JOIN `loglevel` ON `log`.`loglevel`=`loglevel`.`id` ".$where."ORDER BY `log`.`id` DESC LIMIT 100") OR DIE(MYSQLI_ERROR($dbl));
while($row = mysqli_fetch_array($result)) {
  ...
  "<div class='col-x-4 col-s-4 col-m-2 col-l-2 col-xl-2'>".($row['postId'] === NULL ? "<span class='italic'>NULL</span>" : "<a href='https://pr0gramm.com/new/".$row['postId']."' target='_blank' rel='noopener'>".$row['postId']."</a><br><a href='/resetpost?postId=".$row['postId']."'>Post zurücksetzen</a>")."</div>".PHP_EOL.
