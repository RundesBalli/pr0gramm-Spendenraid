 ...
 * fakepostsvalue.php
 * 
 * Anzeige aller Posts, die die selben Beträge und Organisationen haben.
 */
 ...
 */
$title = "Duplikatfinder";
$content.= "<h1>Duplikatfinder</h1>";

/**
 * Alle gleichen Werte finden (Höhe, Breite, Spendenbetrag, Organisation)
 */
$result = mysqli_query($dbl, "SELECT COUNT(`id`) AS `k`, `height`, `width`, `confirmedValue`, `confirmedOrgaId` FROM `items` WHERE `isDonation`='1' AND ((`extension` != 'gif' AND `extension` != 'mp4') AND (`confirmedOrgaId` IS NOT NULL AND `confirmedOrgaId`!='9')) GROUP BY `confirmedValue`, `confirmedOrgaId` HAVING `k`>1 AND `k`<=7 ORDER BY `k` DESC") OR DIE(MYSQLI_ERROR($dbl));

/**
 * Innerhalb dieser zusammengefassten Werte alle Posts ausfindig machen und ausgeben
 */
while($row = mysqli_fetch_array($result)) {
  $query = "SELECT * FROM `items` WHERE `confirmedValue`='".$row['confirmedValue']."' AND `confirmedOrgaId`='".$row['confirmedOrgaId']."' ORDER BY `postId` ASC";
