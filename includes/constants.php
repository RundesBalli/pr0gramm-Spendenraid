...
const FAKE_QUERYS = [
  ...
  [
    'query' => 'SELECT COUNT(`id`) AS `k`, `height`, `width`, `confirmedValue`, `confirmedOrgaId` FROM `items` WHERE `isDonation`="1" AND ((`extension` != "gif" AND `extension` != "mp4") AND `confirmedOrgaId` IS NOT NULL) GROUP BY `confirmedValue`, `confirmedOrgaId` HAVING `k`>1 ORDER BY `k` DESC',
